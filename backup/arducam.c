...
void jpeg_capture_transfer(uint8_t *imgDat, void (*transfer_fn)(uint8_t*,uint32_t)){
    ...
    while(!get_bit(ARDUCHIP_TRIG,CAP_DONE_MASK));
    jpeg_len = read_fifo_length();
    cs_select();
    set_fifo_burst();
    //TODO: consider add pico dma support (level low)
    //pi_read_blocking(SPI_PORT,BURST_FIFO_READ,)
    while (jpeg_len != 0){
        jpeg_send_len = (jpeg_len >=MAX_JPEG_BUF_SIZE) ? MAX_JPEG_BUF_SIZE: jpeg_len;
        pic_buf = jpeg_buf;
        //read buf from spi
        spi_read_blocking(SPI_PORT,BURST_FIFO_READ,pic_buf,jpeg_send_len);
        jpeg_len -= jpeg_send_len;
        //send via uart
        transfer_fn(pic_buf,jpeg_send_len);
    }
}
#endif
